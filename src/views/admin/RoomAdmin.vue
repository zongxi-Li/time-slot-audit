...
function toggleStatus(room: MeetingRoom) {
  ...
  if (!next) return
  monitor.log(
    next === 'active' ? '启用会议室' : '停用会议室',
    `${room.name} · ${room.location}${next === 'disabled' ? '（停用后不可新建预约，已有预约保留）' : ''}`,
    '王建国',
    'admin',
  )
  monitor.pushFeed({
    ...
    status: 200,
    user: '管理员',
    note: `${next === 'active' ? '启用' : '停用'} ${room.name}`,
  })
  ElMessage.success(next === 'active' ? `${room.name} 已启用` : `${room.name} 已停用`)
}
</script>
