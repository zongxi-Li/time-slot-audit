...
| `reservation` **新增** `version` | 同一预约可在多个浏览器标签或设备上被修改；会议室行锁只保证时段冲突安全，不能避免后写覆盖。`PUT` 携带版本并在条件 UPDATE 中校验，写入成功后递增。 |
...
| version | INT | 否 | 0 | 乐观锁版本；每次预约写操作递增 |
...
| status | VARCHAR(20) | 否 | —（无默认值） | 持久化状态机：`PENDING` / `CONFIRMED` / `REJECTED` / `CANCELLED`。`COMPLETED` 是由 `CONFIRMED + now >= end_time` 推导的展示终态，不写入数据库。**不设 DB 默认值**：初始状态由分类的 `approval_required` 决定，是业务决策，不是存储默认值 |
| remark | VARCHAR(500) | 是 | NULL | 申请人备注 |
| cancel_reason | VARCHAR(500) | 是 | NULL | 取消原因：用户取消 / 管理员强制取消时填写 |
| created_at / updated_at | DATETIME | 否 | CURRENT_TIMESTAMP | 同上 |

**候选字段的取舍结论**：`reservation_no` 保留（追溯展示）；`cancel_reason` 保留（取消语义必需）；`version` 保留（防止同一预约被并发覆盖）；`deleted`、`approved_at` 不保留（理由见 2.1）。
...
持久化状态集合：`PENDING` / `CONFIRMED` / `REJECTED` / `CANCELLED`。`UPCOMING`、`IN_USE`、`COMPLETED` 均为服务端按统一时钟推导的展示状态，不引入 `APPROVED` 等同义状态。
...
| `CONFIRMED` | 当前时间达到结束时间 | 展示为 `COMPLETED` | 不写库；`COMPLETED` 展示终态不可再强制取消 |
| `REJECTED` / `CANCELLED` | 不可迁移 | — | 持久化终态 |

补充约束：

- `PENDING` 与 `CONFIRMED` **都占用时间段**并参与冲突检测（审批期间锁段，避免审批通过后撞车）。
- 状态迁移在 SQL 层一律用**条件更新**保证原子性，例如审批通过：
  `UPDATE reservation SET status='CONFIRMED', version=version+1 WHERE id=? AND status='PENDING'`，影响行数=1 才算成功，天然防止重复审批和并发迁移；改期还须校验客户端携带的 version。
- `COMPLETED` 由读取时统一计算，不引入定时任务或懒写回；前端不得按本机时钟自行重算。
...
即**半开区间 `[start, end)` 求交**：首尾相接不冲突，任何重叠即冲突。`REJECTED`、`CANCELLED` 不参与判断；`COMPLETED` 从不作为持久化状态参与查询。
...
| 预约 | 覆盖全部 4 种持久化状态：B201 `CONFIRMED`（普通，免审批直接确认）、B502 `PENDING`（大型，待审批）、S101 `CONFIRMED`+`REJECTED`（特殊，含审批历史）、A301 `CANCELLED`（管理员强制取消，带原因）；已结束预约以 `CONFIRMED` 读取时展示为 `COMPLETED` |
